...
let lines = inputStrings
    .map({ string -> Line in
        ...
        return Line(
            digits: splitted[0].split(separator: " ").map({ String($0.sorted(by: <)) }),
            values: splitted[1].split(separator: " ").map({ String($0.sorted(by: <)) })
        )
    })
...
let result = lines
    .map({ $0.values })
    .flatMap({ $0 })
    .filter({ lenghts.contains($0.count) })
    .count
...
func contains(first: String, second: String?) -> Bool {
    return second?.reduce(true, { $0 && first.contains($1) }) ?? false
}

func makeDict(digits: [String]) -> [String: Int] {

    func setPattern(int: Int, string: String) {
        stringDict[string] = int
        intDict[int] = string
    }

    let digits = digits.sorted(by: { $0.count < $1.count })
    var stringDict: [String: Int] = [:]
    var intDict: [Int: String] = [:]

    for digit in digits {
        switch digit.count {
        case 2: setPattern(int: 1, string: digit)
        case 3: setPattern(int: 7, string: digit)
        case 4: setPattern(int: 4, string: digit)
        case 7: setPattern(int: 8, string: digit)

        case 5:
            if contains(first: digit, second: intDict[1]) {
                setPattern(int: 3, string: digit)
            }
            else {
                let fourWithoutOne = intDict[4]?.filter({ intDict[1]?.contains($0) == false })
                contains(first: digit, second: fourWithoutOne)
                    ? setPattern(int: 5, string: digit)
                    : setPattern(int: 2, string: digit)
            }


        case 6:
            if contains(first: digit, second: intDict[1]) {
                contains(first: digit, second: intDict[4])
                    ? setPattern(int: 9, string: digit)
                    : setPattern(int: 0, string: digit)
            }
            else {
                setPattern(int: 6, string: digit)
            }
...
func makeNumber(digits: [Int]) -> Int {
    ...
    for (index, digit) in digits.enumerated() {
        result += digit * Array(repeating: 10, count: digits.count - index - 1).reduce(1, { $0 * $1 })
    }
    return result
}
...
var sum = 0
for line in lines {
    let dict = makeDict(digits: line.digits)
    sum += makeNumber(digits: line.values.map({ dict[$0] ?? 0 }))
}
print(sum)
